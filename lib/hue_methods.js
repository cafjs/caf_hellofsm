...
exports.methods = {
    ...
    async hue_setDaemon(daemon) {
        const hue_daemon = daemon ?
            this.$.session.getSessionId() :
            null;
        if (this.state.hue_daemon && hue_daemon &&
            (this.state.hue_daemon !== hue_daemon)) {
            return [new Error('Cannot set daemon')];
        } else {
            if (this.state.hue_daemon && !hue_daemon) {
                this.hue_reset();
            }
            this.state.hue_daemon = hue_daemon;
            notifyHueApp(this, 'Changing daemon');
            return this.getState();
        }
    },
