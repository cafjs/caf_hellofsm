...
exports.methods = {
    async __ca_init__() {
        ...
        this.scratch.fuzzyOffset = Math.ceil(Math.random()*MAX_DELAY);
    ...
    async __ca_pulse__() {
        ...
        if (this.state.linkedTo) {
            ...
            this.$.log && this.$.log.debug(`delay:${this.scratch.fuzzyOffset}`);
            // Spread pulses in one process to make it obvious not synchronized
            await setTimeoutPromise(this.scratch.fuzzyOffset);
            await this.tick();
            // if linkedTo render in `__ca_handleTick__`
