...
const hue = require('./hue_methods.js');
...
const APP_SESSION = exports.APP_SESSION = /^user/; // app with no IoT

const notifyWebApp = function(self, msg) {
    self.$.session.notify([myUtils.deepClone(self.state)], APP_SESSION);
};
...
const methods = {
    async __ca_init__() {
        ...
        // initialize IoT methods
        this.__ca_hue_init__();
        return [];
    },
    ...
    async __ca_createMachine__() {
        const updateImpl = (newLight) => {
            ...
            notifyWebApp(this, `New color ${newLight}`);
            this.hue_setColor(newLight);
...
exports.methods = {...methods, ...hue.methods};
